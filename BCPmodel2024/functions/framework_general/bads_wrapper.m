...
% (i.e. lowest first. Function values will be minimized by BADS, so smallest values are "best")
[~,idx_sorted] = sort(LP_gridSearch);
...
% Attempt to optizime the parameters, repeat minimally nAttempts(1) times and, if necessary, 
% maximally nAttempts(2) times   
...
while ~BADSconvergedBool && (nFinished < nAttempts(2))
    ...
    [fittedParams(nFinished,:), ...
        fittedValue(nFinished), ...
        exitflags(nFinished), ...
        outputstructs{nFinished,1}] = ...
            bads(funwrapper,params0(nFinished,:),LB,UB,PLB,PUB,nonbcon,options);
